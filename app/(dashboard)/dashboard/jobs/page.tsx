  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-5">
        <div className="flex items-center gap-1 p-1 bg-[#141416] border border-[#1f1f28] rounded-lg w-fit">
          {(["active", "archived"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors capitalize ${
                tab === t ? "bg-[#1f1f28] text-white" : "text-zinc-500 hover:text-zinc-300"
              }`}
            >
              {t}
            </button>
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 bg-[#141416] border border-[#1f1f28] rounded-lg text-sm text-zinc-300 placeholder-zinc-600 focus:outline-none focus:border-orange-500/50"
            />
          </div>
          <Link
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-[#141416] border border-[#1f1f28] rounded-xl">
          ...
          <div className="bg-[#141416] border border-[#1f1f28] rounded-xl overflow-hidden">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[#1f1f28]">
                {filtered.map((job) => (
                  <tr
                    key={job.id}
                    className="hover:bg-[#1a1a22] transition-colors group"
                  >
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {job.status === "active" ? (
                          ...
                          <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1.5 text-xs text-zinc-400 hover:text-white bg-[#1f1f28] hover:bg-[#2a2a36] rounded-md transition-colors"
                            title="Unarchive"
                          >
                        ...
                        <Link
                          href={`/dashboard/jobs/${job.id}`}
                          className="flex items-center gap-1 px-3 py-1.5 text-xs text-zinc-300 hover:text-white bg-[#1f1f28] hover:bg-[#2a2a36] rounded-md transition-colors"
                        >
